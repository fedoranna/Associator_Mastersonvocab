...
% when model learns right islands from 10% of items with bias and permutation

%%

function M = matfileanalyzator_phases_islands_AT(matfile, param)
...
for i = 1:length(T.scores)
    if T.scores(i) > min(T.scores)-min(T.scores)*0.01 && T.scores(i) < min(T.scores)+min(T.scores)*0.01
        p1_2 = i-1;
    end
end
